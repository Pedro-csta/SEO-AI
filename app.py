...
try:
    ...
    st.warning("Chave da API Gemini não encontrada. A análise por IA está desabilitada.", icon="⚠️")
...
def check_broken_links(base_url: str, internal_links: list) -> list:
    ...
    for link in internal_links:
        # Constrói a URL absoluta para ter certeza que podemos testá-la
        full_url = urljoin(base_url, link)
        try:
            # Usamos HEAD para ser mais rápido e não baixar o corpo da página
            response = requests.head(full_url, headers=headers, timeout=5, allow_redirects=True)
            # Consideramos erro qualquer status 4xx (cliente) ou 5xx (servidor)
            if response.status_code >= 400:
                broken_links.append({"url": full_url, "status": response.status_code})
        except requests.RequestException:
            # Se a conexão falhar, também consideramos "quebrado"
...
def onpage_checks(url):
    ...
    # (O resto desta função continua igual à versão anterior)
    try:
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
        }
    ...
    # Título, Meta Description, H1, etc... (código inalterado)
    ...
    # Filtra links que não são âncoras vazias, de telefone, ou email
    ...
    # Imagens, Contagem de palavras, etc... (código inalterado)
    ...
    # Retornamos os checks e a lista de links internos para a nova função
    return checks, internal_links


def generate_gemini_recommendations(checks, url):
    """Gera recomendações de SEO usando Google Gemini. (Função inalterada)"""
    if not GEMINI_API_KEY:
        return "A análise por IA está desabilitada pois a chave da API do Gemini não foi configurada."
        
    report_details = "\n".join([f"- {key.replace('_', ' ').title()}: {value}" for key, value in checks.items()])
    prompt = f"""
    Você é um especialista sênior em SEO... (prompt completo da versão anterior)
    """
...
if st.button("🛰️ Rodar Auditoria Completa", type="primary"):
    if not url.startswith("http"):
        ...
        try:
            ...
            # --- Etapa 2: Análises Técnicas (Pilar 2) ---
            ...
            # PAINEL DE LINKS QUEBRADOS (NOVO)
            ...
            # PAINEL ON-PAGE (EXISTENTE)
            st.subheader("📊 Painel de Auditoria On-Page")
            # Tabela, colunas e expander da versão anterior...
            df = pd.DataFrame({
                "Elemento": ["Título", "Meta Description", "H1 (Primeiro)"],
                "Conteúdo": [onpage_results.get("title", ""), onpage_results.get("meta_description", ""), onpage_results.get("h1_text", "")]
            })
            st.table(df)
            ...
            with st.spinner("Etapa 3/3: A IA está processando todos os dados para criar as melhores recomendações..."):
                gemini_sug = generate_gemini_recommendations(onpage_results, url)
